...
contract ArcadeToken is ERC20, ERC20Detailed {      // is statement inherits the erc20 & erc20detailed and that ArcadeToken contract is a version of both
    ...
    constructor(uint initial_supply) ERC20Detailed("ArcadeToken", "ARCD", 18) public {      //3 parms are name, ticker and number of decimals
        owner = msg.sender;                                                                 //18 decimals for ether
        _mint(owner, initial_supply);   //2 parms address and amount, _mint is the internal mint function specific to erc20
    }

    function mint(address recipient, uint amount) public onlyOwner {       //this allows to mint tokens after initial supply mint upon deploy
        _mint(recipient, amount);       //_mint is the internal mint function specific to erc20
    }
}